...
const beneficiaryAccountId = ref<string>("");
...
const createTransaction = async () => {
  await axios
    .post<Transaction>(
      "/api/accounts/" +
        principalAccount.value.account_id.replace(/^0+/, "") +
        "/transactions",
      {
    ...
    .then(async (response) => {
      ...
      if (isJson) {
        notificationMessage.value =
          "Created Transaction with key " + data.transaction_id;
        errorMessage.value = "";
      }
    })
    .catch((error) => {
...
const findBeneficiaryAccount = () => {
  const accountId = beneficiaryAccountId.value.replace(/^0+/, "");
  const account = accounts.value.filter(
    (a) => a["account_id"].replace(/^0+/, "") === accountId,
  );
  if (account.length > 0) {
...
const executeTransaction = async () => {
  ...
  if (
    ...
    principalAccount.value.account_id === ""
  ) {
    errorMessage.value = "Cannot proceed: Principal Account not defined";
    return;
  }
  findBeneficiaryAccount();
  if (
    beneficiaryAccount === undefined ||
    beneficiaryAccount.value.account_id === ""
  ) {
    errorMessage.value =
      "Cannot proceed: Beneficiary Account not valid or not existent";
    return;
  }
  if (!isMoneyValue(amount.value)) {
            ...
            <v-card rounded="0">
              <v-card-text>
                <v-form @submit.prevent>
                  ...
                  ></v-text-field>
                  <div class="mt-5"></div>
                  <v-btn @click="executeTransaction" type="submit" block>
                    Proceed
                  </v-btn>
                </v-form>
              </v-card-text>
